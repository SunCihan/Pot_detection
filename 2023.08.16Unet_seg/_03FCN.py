...
class Net(nn.Module):
	...
	def Block(self, InputChannels, OutputChannels, Name):
		return nn.Sequential(OrderedDict([
			(Name + "_Conv1", nn.Conv2d(in_channels=InputChannels, out_channels=OutputChannels, kernel_size=7, padding=3, bias=False)),
			(Name + "_Norm1", nn.BatchNorm2d(num_features=OutputChannels)),
			(Name + "_Relu1", nn.ReLU(inplace=True)),
			# (Name + "_Drop1", nn.Dropout2d(0.5)),
			(Name + "_Conv2", nn.Conv2d(in_channels=OutputChannels, out_channels=OutputChannels, kernel_size=7, padding=3, bias=False)),
